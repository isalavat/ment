		...
		return this.transaction.run(async () => {
			...
			if (input.action === "verify") {
				if (mentor.categories.length === 0) {
					throw new BadRequestError("Mentor must have at least one category before being verified");
				}
				if (mentor.skills.length === 0) {
					throw new BadRequestError("Mentor must have at least one skill before being verified");
				}
				if (mentor.hourlyRate <= 0) {
					throw new BadRequestError("Mentor must have a valid hourly rate before being verified");
				}
				return this.mentorRepo.verifyMentor(input.mentorId, "VERIFIED");
			}

			return this.mentorRepo.verifyMentor(input.mentorId, "REJECTED", input.rejectionReason);
		});
	}
}
