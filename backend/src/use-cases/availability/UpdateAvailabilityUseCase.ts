...
import { ensureStartBeforeEnd, ensureValidDayOfWeek, ensureValidTimeFormat } from "./validators";
...
export class UpdateAvailabilityUseCase {
	...
	async execute(input: UpdateAvailabilityInput) {
		...
		const updateData: Prisma.AvailabilityUpdateInput = {
			...(input.data.dayOfWeek !== undefined ? { dayOfWeek: input.data.dayOfWeek } : {}),
			...(input.data.startTime !== undefined ? { startTime: input.data.startTime } : {}),
			...(input.data.endTime !== undefined ? { endTime: input.data.endTime } : {}),
			...(input.data.isRecurring !== undefined ? { isRecurring: input.data.isRecurring } : {}),
			...(input.data.specificDate !== undefined ? { specificDate: input.data.specificDate } : {}),
		};
