...
export class CreateAvailabilityUseCase {
	...
	async execute(input: CreateAvailabilityInput) {
		const { mentorId, dayOfWeek, startTime, endTime, isRecurring = true, specificDate } = input;
		...
		if (isRecurring && dayOfWeek === undefined) {
			throw new BadRequestError("dayOfWeek is required for recurring availability");
		}

		if (!isRecurring && !specificDate) {
			throw new BadRequestError("specificDate is required for non-recurring availability");
		}
