...
export class GetAllSlotsForMentorUseCase {
	...
	async execute(mentorId: string, startDate?: Date, endDate?: Date, status?: SlotStatus) {
		return this.timeSlotRepo.findAllForMentor(mentorId, startDate, endDate, status);
	}
}
