...
export class UpdateUserUseCase {
	...
	async execute(userId: string, input: UpdateUserInput): Promise<User> {
		return this.transaction.run(async () => {
			...
			const updatedEmail = input.email ? Email.from(input.email) : current.email;
			const updatedPassword = input.password ? await this.passwordHasher.hash(input.password) : current.hashedPassword;
