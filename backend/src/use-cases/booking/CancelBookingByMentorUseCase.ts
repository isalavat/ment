...
export class CancelBookingByMentorUseCase {
	...
	async execute(bookingId: string, mentorId: string): Promise<BookingCreatedRecord> {
		return this.transaction.run(async () => {
			...
			return this.bookingRepository.updateStatus(bookingId, "CANCELLED_BY_MENTOR", "cancelledAt");
		});
	}
}
