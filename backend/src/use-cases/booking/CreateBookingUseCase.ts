...
import type { TimeSlotRecord, TimeSlotRepository } from "../../domain/timeSlot/TimeSlotRepository";
...
function isWithinAvailabilityWindow(
	availability: { isRecurring: boolean; specificDate: Date | null; startTime: string; endTime: string },
	startTime: Date,
	endTime: Date,
) {
	if (!availability.isRecurring && availability.specificDate?.toDateString() !== startTime.toDateString()) {
		return false;
	}
...
export class CreateBookingUseCase {
	...
	async execute(input: CreateBookingUseCaseInput): Promise<BookingCreatedRecord> {
		...
		return this.transaction.run(async () => {
			...
			let timeSlot = input.timeSlotId
				...
				: input.startTime && input.endTime
					? await this.timeSlotRepository.findByMentorAndRange(input.mentorId, input.startTime, input.endTime)
					: null;

			if (!timeSlot && input.startTime && input.endTime) {
				const { dayStart, dayEnd } = buildAvailabilityWindow(input.startTime);
				const availabilities = await this.availabilityRepository.findForMentorInRange(input.mentorId, dayStart, dayEnd);

				const withinAvailability = availabilities.some((availability) =>
					isWithinAvailabilityWindow(availability, input.startTime as Date, input.endTime as Date),
				);
				...
				const [blockedSlot, activeBooking] = await Promise.all([
					this.timeSlotRepository.findBlockedOverlap(input.mentorId, input.startTime, input.endTime),
					this.bookingRepository.findActiveOverlap(input.mentorId, input.startTime, input.endTime),
				]);
