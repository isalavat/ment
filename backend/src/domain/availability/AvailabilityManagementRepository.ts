...
export interface AvailabilityManagementRepository {
	...
	createWeekly(data: CreateWeeklyAvailabilityRecord): Promise<{ count: number }>;
	...
	findRecurringForMentor(mentorId: string, dayOfWeek?: number): Promise<AvailabilityDetailsRecord[]>;
	findSpecificDateForMentor(mentorId: string, date?: Date): Promise<AvailabilityDetailsRecord[]>;
	update(id: string, data: Prisma.AvailabilityUpdateInput): Promise<AvailabilityDetailsRecord>;
