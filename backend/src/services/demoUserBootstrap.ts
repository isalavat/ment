...
async function ensureDemoMentorState(
	...
	if (mentorState === "PENDING_PROFILE") {
		...
		await prisma.availability.deleteMany({ where: { mentorId: mentorProfile.id } });
		await prisma.mentorCategory.deleteMany({ where: { mentorId: mentorProfile.id } });
		await prisma.mentorSkill.deleteMany({ where: { mentorId: mentorProfile.id } });
		return;
	}
