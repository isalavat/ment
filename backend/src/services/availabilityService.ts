...
export const availabilityService = {
	...
	async createAvailability(data: CreateAvailabilityData) {
		const { mentorId, dayOfWeek, startTime, endTime, isRecurring = true, specificDate } = data;
	 ...
	 */
	async updateAvailability(id: string, mentorId: string, data: UpdateAvailabilityData) {
		const availability = await prisma.availability.findUnique({
			where: { id },
		});
