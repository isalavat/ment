...
export class TimeSlotGenerationPrismaService implements TimeSlotGenerationService {
	async generateTimeSlots(input: GenerateTimeSlotsInput): Promise<{ count: number; message: string }> {
		...
		for (const currentDate of this.iterateDays(startDate, endDate)) {
			const daySlots = this.buildSlotsForDate(currentDate, availabilities, mentorId, slotDuration);
		...
		const newSlots = slotsToCreate.filter(
			(newSlot) => !existingSlots.some((existing) => existing.startTime.getTime() === newSlot.startTime.getTime()),
		);
	...
	async getComputedBookableSlots(input: ComputeBookableSlotsInput): Promise<unknown[]> {
		const { mentorId, startDate, endDate, stepMinutes = 15, durationMinutes = 60 } = input;
		...
		if (stepMinutes <= 0 || durationMinutes <= 0) {
			throw new BadRequestError("stepMinutes and durationMinutes must be positive");
		}
	...
	private buildSlotsForDate(
		...
		for (const availability of matchedAvailabilities) {
			slots.push(
				...this.generateSlotsForDay(date, availability.startTime, availability.endTime, mentorId, slotDuration),
			);
		}
