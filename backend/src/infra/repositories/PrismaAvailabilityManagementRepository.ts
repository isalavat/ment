...
export class PrismaAvailabilityManagementRepository implements AvailabilityManagementRepository {
	async create(data: CreateAvailabilityRecord): Promise<AvailabilityDetailsRecord> {
	...
	async createWeekly(data: CreateWeeklyAvailabilityRecord): Promise<{ count: number }> {
	...
	async findByIdWithMentor(id: string): Promise<AvailabilityDetailsRecord | null> {
		const availability = await PrismaClientGetway().availability.findUnique({
			where: { id },
	...
	async findRecurringForMentor(mentorId: string, dayOfWeek?: number): Promise<AvailabilityDetailsRecord[]> {
		return PrismaClientGetway().availability.findMany({
			where: {
				mentorId,
				isRecurring: true,
	...
	async findSpecificDateForMentor(mentorId: string, date?: Date): Promise<AvailabilityDetailsRecord[]> {
		if (!date) {
			return PrismaClientGetway().availability.findMany({
				where: {
					mentorId,
	...
	async update(id: string, data: Prisma.AvailabilityUpdateInput): Promise<AvailabilityDetailsRecord> {
	...
	private toAvailabilityWithMentor(availability: AvailabilityWithMentor): AvailabilityDetailsRecord {
		return {
			id: availability.id,
